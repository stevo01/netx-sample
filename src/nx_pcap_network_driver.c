...
struct sched_param sp;
    ...
    if((nx_pcap_fp = pcap_create(nx_pcap_source_name, NULL)) == NULL)
    {
        ...
        printf("pcap_create passed device name is %s\n", nx_pcap_source_name);
    }
    ...
    int status = pcap_activate(nx_pcap_fp);
    if (status < 0)
    {
        if(status == -8)
        {
            printf("pcap_activate failed with status=%d no permission to open the device \n", status); 
        }
        else if( status == -5)
        {
            printf("pcap_activate failed with status=%d no such device exists \n", status);   
        }
        else {
            printf("pcap_activate failed with status=%d \n", status);    
        }
        nx_pcap_cleanup();
        return NX_NOT_CREATED;
    }
        ...
        case NX_LINK_INITIALIZE:
            {
                ...
                status = nx_ip_interface_mtu_set(ip_ptr, interface_index, (NX_LINK_MTU - NX_ETHERNET_SIZE));
                if(status != NX_SUCCESS)
                {
                    printf("error: nx_ip_interface_mtu_set status=0x%x\n", status);
                }
                ...
                /* For this pcap driver, the MAC address is constructed by 
                   ...
                   ethernet.  */
                status = nx_ip_interface_physical_address_set(ip_ptr, interface_index, 
                        nx_pcap_address_msw,
                        nx_pcap_address_lsw, 
                        NX_FALSE);
                ...
                if(status != NX_SUCCESS)
                {
                    printf("error: nx_ip_interface_physical_address_set status=0x%x\n", status);
                }

                /* Indicate to the IP software that IP to physical mapping is required.  */
                status = nx_ip_interface_address_mapping_configure(ip_ptr, interface_index, NX_TRUE);

                if(status != NX_SUCCESS)
                {
                    printf("error: nx_ip_interface_address_mapping_configure status=0x%x\n", status);
                }
#else 
...
#endif
                status = _nx_pcap_initialize(ip_ptr); 
                if(status != NX_SUCCESS)
                {
                    printf("error: _nx_pcap_initialize status=0x%x\n", status);
                }
